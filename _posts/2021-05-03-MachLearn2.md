...
기계학습은 다음 서적을 참고하여 정리하였다. 책에 있는 모든 내용이 있는것이 아니니 자세한 내용은 책을 참고하길 바란다.
...
$\mathbf{x} = \big ( \begin{smallmatrix} x_1 \\ x_2 \\ x_3 \\ x_4 \end{smallmatrix} \big ) = \big ( \begin{smallmatrix} 5.1 \\ 3.5 \\ 1.4 \\ 0.2 \end{smallmatrix} \big )$  
...
$\mathbf{X} = \big ( \begin{smallmatrix} 5.1 & 5.2 & 5.3 \\ 3.5 & 3.6 & 3.7\\ 1.4 & 1.5 & 1.6 \\ 0.2 & 0.3 & 0.4 \end{smallmatrix} \big )$  
...
- $\mathbf{A} = \big ( \begin{smallmatrix} 3 & 4 & 1 \\ 0 & 5 & 1 \end{smallmatrix} \big )$ 이라면 $\mathbf{A^T} = \big ( \begin{smallmatrix} 3 & 0 \\ 4 & 5 \\ 5 & 1 \end{smallmatrix} \big )$ 로 표현한다.  

정사각행렬  
행과 열의 개수가 같은 행렬을 말한다.  
- $\big ( \begin{smallmatrix} 2 & 0 & 1 \\ 1 & 21 & 5 \\ 4 & 5 & 12 \end{smallmatrix}\big )$  

대각행렬  
주 대각선을 제외한 요소가 모두 0인 행렬을 말한다.  
- $\big ( \begin{smallmatrix} 50 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 12 \end{smallmatrix}\big )$  

단위행렬  
주 대각선 요소가 1이고 나머지 요소가 모두 0인 정사각행렬을 말한다.  
- $\big ( \begin{smallmatrix} 1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{smallmatrix}\big )$  

대칭행렬  
- $\big ( \begin{smallmatrix} 1 & 2 & 11 \\ 2 & 21 & 5 \\ 11 & 5 & 1 \end{smallmatrix}\big )$  
---  

##### 행렬의 연산  

- 덧셈 : 해당하는 요소끼리 더하면 된다.  
$\big ( \begin{smallmatrix} 1 & 2 \\ 2 & 5 \end{smallmatrix}\big )$ + $\big ( \begin{smallmatrix} 2 & 1 \\ 2 & 6 \end{smallmatrix}\big )$ = $\big ( \begin{smallmatrix} 3 & 3 \\ 4 & 11 \end{smallmatrix}\big )$  
...
* $cosine\_similarity(\mathbf{a,b}) = {\mathbf{a} \over ||\mathbf{a}||} \cdot {\mathbf{b} \over ||\mathbf{b}||} = cos(\theta)$  
...
$\mathbf{o} = \mathbf{r} \big ( \begin {smallmatrix} \mathbf{w_1 \cdot x} \\ \mathbf{w_2 \cdot x} \\ \cdots \\ \mathbf{w_c \cdot x}  \end {smallmatrix} \big )$  $= \mathbf{r(Wx)}$  
...
* $\big ( \begin {smallmatrix} 1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1   \end {smallmatrix} \big )$ : 선형 독립 행렬 3개, 계수 3  (벡터 공간은 3차원 공간 전체)

* $\big ( \begin {smallmatrix} 1 & 0 & 0 \\ 0.8 & 1 & 0.8 \\ 0 & 0 & 1   \end {smallmatrix} \big )$ : 선형 독립 행렬 2개, 계수 2 (벡터 공간은 평면)

* $\big ( \begin {smallmatrix} 0 & 0 & 4 \\ 0 & 0 & 0.1 \\ 0 & 0 & 1.2   \end {smallmatrix} \big )$ : 선형 독립 행렬 1개, 계수 1 (벡터 공간은 직선)  
...
예를 들어, $( \begin {smallmatrix} 1 & 0 \\ 0 & 2 \end {smallmatrix})$ 는 $( \begin {smallmatrix} x_1 & x_2 \end {smallmatrix})$$( \begin {smallmatrix} 1 & 0 \\ 0 & 2 \end {smallmatrix})$$( \begin {smallmatrix} x_1 \\ x_2 \end {smallmatrix})$ 이므로 $\mathbf{x} = (x_1 x_2)^T \not = ( \begin {smallmatrix} 0 & 0 \end {smallmatrix})$인 이상 항상 0보다 크므로 이 행렬은 양의 정부호 행렬이다.  

반면 부정부호 행렬은 벡터에 따라 음수가 되기도 하고 양수가 되기도 한다.  
