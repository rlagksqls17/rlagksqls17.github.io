...
반면 나머지 변수들('Calories', 'Sugars (g)', 'Caffeine (mg)')은 양적 변수로 카운트 혹은 측정값을 나타낸다. 범주형 변수는 측정값이 아니라는 점에서 양적변수와 다르다. 또한 범주형 변수와 양적 변수를 하나의 데이터로 가지는 각각의 개체(Brewed coffee, Caffe latte, ...)를 식별자(identifial)라 한다.
